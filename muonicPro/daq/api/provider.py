...
BaseDAQProvider shall give the base structure of the  
...
# from exception.py
from muonicPro.daq.exceptions import DAQIOError, DAQMissingDependencyError
...
class DAQProvider(BaseDAQProvider):
    ...
    def __init__(self, logger=None, sim=False):
        ...
        # app finishes
        # The deamon to read from daq card
        self.read_thread = multiprocessing.Process(target=self.daq.read, name="pREADER")
        self.read_thread.daemon = True
        self.read_thread.start()

        # The deamon to write to the daq card
        if not sim:
            self.write_thread = multiprocessing.Process(target=self.daq.write,
                                           name="pWRITER")
